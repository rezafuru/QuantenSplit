...
from misc.util import Detokenizer, Tokenizer
...
def conv1x1(in_ch: int,
            ...
            stride: int = 1,
            bias: bool = True,
            upsample: bool = False) -> Union[nn.Conv2d, nn.ConvTranspose2d]:
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        # divide x into 4 separate parts each with H/2 W/2 dims
        #  then "stack" them on top of each other, i.e. divides resolution by 4 and increases channels by 4
        x_cat = torch.cat([x0, x1, x2, x3], -1)  # B H/2 W/2 4*C
        x_cat = x_cat.view(B, -1, 4 * C)  # B H/2*W/2 4*C

        x_merge = self.norm(x_cat)
        x_reduced = self.reduction(x_merge)
        return x_reduced
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.Identity, drop=0.):
        super().__init__()
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, add_token=False):
        """
        Args:
...
class BasicSwinStage(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        sample (nn.Module | None, optional): Downsample layer at the end of the layer. Default: None
    ...
    def __init__(self,
                 ...
                 sample=None,
                 pre_sample=False,
                 transformer_block=SwinTransformerBlock,
        ...
        self.blocks = nn.ModuleList([
            transformer_block(dim=dim if block_dim is None else block_dim,
        ...
        # patch merging layer (either down or upsample)
        self.sample = sample(
            input_resolution,
            dim=dim,
            out_dim=out_dim,
            norm_layer=norm_layer) if sample is not None else nn.Identity()
    ...
    def update_resolution(self, H, W):
        ...
        if self.sample is not None:
            self.sample.input_resolution = (H, W)
...
class SwinReconLayer(BasicSwinStage):
    def __init__(self,
        ...
        assert sample in ["downsample", "upsample"]
        ...
        self.detokenizer = Detokenizer(spatial_dims=self.output_size) if detokenize_output else nn.Identity()
...
class HybridSwinStage(nn.Module):
    """
        Replaces PatchMerging with a down or upsampling (de-)convolutonal layer
    """
    ...
    def __init__(self,
                 ...
                 use_shortcut=True,
                 use_checkpoint=False):
        super(HybridSwinStage, self).__init__()
        ...
        self.detokenizer = Detokenizer(self.input_resolution)
